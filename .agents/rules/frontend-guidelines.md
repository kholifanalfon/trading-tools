...
4.  **Shadcn UI Specification**: WAJIB menggunakan komponen Shadcn UI saat membuat UI/design (seperti button, dialog, table, card, form inputs, dll.) untuk menjaga konsistensi visual dan standar aksesibilitas komponen.
5.  **Flat Clean Design**: Utamakan desain minimalis, flat, dan bersih (clean design) dengan visual hierarchy yang jelas, kontras warna yang baik, ruang kosong (whitespace) yang cukup, serta menghindari shadow, gradien, atau ornamen 3D yang berlebihan.
