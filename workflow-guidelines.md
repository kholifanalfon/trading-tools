...
```
LANGKAH 1 → Definisikan/update schema di apps/backend/src/db/schema.ts
LANGKAH 2 → Jalankan: bun run db:generate
LANGKAH 3 → Review file migrasi yang dihasilkan di apps/backend/src/db/migrations/
LANGKAH 4 → Jalankan: bun run db:migrate
```

> **PERINGATAN:** JANGAN PERNAH mengedit file migrasi yang sudah ada. SELALU generate baru.

### 1.4 Membuat Migration Generator (Custom)

Untuk migration yang membutuhkan logika programatik (bukan hanya schema change):
